...
public interface ProductRepository extends JpaRepository<Product, Long>, JpaSpecificationExecutor<Product>, PagingAndSortingRepository<Product, Long> {
	...
	Product getByProductNameAndModel(String type, String name, String model);
	...
	@Query("SELECT DISTINCT FUNCTION('JSON_EXTRACT', p.details, CONCAT('$.', :configurationType)) FROM Product p WHERE p.type = :type")
	List<String> findConfigurationType(@Param("configurationType") String configurationType, @Param("type") String type);
}
