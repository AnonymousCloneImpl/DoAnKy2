...
const ProductsByTypePage = () => {
    ...
    const apiBE = `${process.env.DOMAIN}/products/${query.type}`;

    const {data, error} = useSWR(apiBE, fetcher);

    useEffect(() => {
        const apiBE = `${process.env.DOMAIN}/products/${query.type}`;
        const fetchData = async () => {
            const result = await fetcher(apiBE);
            if (result !== undefined && result !== null && result.productSummaryDtoList.length > 0) {
                setProducts(result.productSummaryDtoList);
                setTotalPage(result.totalPageNumber);
                setElementPerPage(result.elementPerPage);
    ...
    useEffect(() => {
        const fetchData = async () => {
            const apiBE = `${process.env.DOMAIN}/products/${query.type}?page=${currentPage}`;
            const result = await fetcher(apiBE);
            setProducts(result.productSummaryDtoList);
        };
    ...
    const fetchNewData = async (page) => {
        ...
        if (page === 1) {
            url = `${process.env.DOMAIN}/products/${query.type}?limit=${elementPerPage}`;
        }
